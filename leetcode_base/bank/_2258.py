...
class _1st:
    def maximumMinutes(self, grid: List[List[int]]) -> int:
        ...
        # 返回能否在初始位置停留t分钟，并安全达到安全屋
        def check(t: int) -> bool:
            return False



# https://leetcode.cn/problems/escape-the-spreading-fire/solutions/1460794/er-fen-bfspythonjavacgo-by-endlesscheng-ypp1/?envType=daily-question&envId=2023-11-09
class _2nd:
    def maximumMinutes(self, grid: List[List[int]]) -> int:
        m, n = len(grid), len(grid[0])
        ...
        def check(t: int) -> bool:
            ...
            while q:
                ...
                for i, j in tmp:
                    ...
                    for x, y in (i - 1, j), (i + 1, j), (i, j - 1), (i, j + 1):  # 上下左右
                        if 0 <= x < m and 0 <= y < n and grid[x][y] == 0 and (x, y) not in on_fire and (
                                x, y) not in vis:
                            if x == m - 1 and y == n - 1:
